...
def processFrame(frame):
    ...
    cl = clahe.apply(l) # TODO: Clahe gives great contrast but increases noise between frames dramatically.
...
def PixelSignal(grey_frames):
  ...
  # pixel_num = 0
    ...
    pixel_signals = np.transpose(as_array, axes=[1, 2, 0]).reshape(-1, as_array.shape[0])
    pixel_signals = dict(enumerate(pixel_signals.tolist()))

    remove = [k for k in pixel_signals if all(v == 0 for v in pixel_signals[k])]
    for k in remove: 
        del pixel_signals[k]
    ...
    if peak_variance < 0.0001:
        ...
        if len(peaks) == 1:
            ...
            if not average_values:
                ...
                LOGGER.info("found " + str(len(peaks)) + " peak(s), and selected the closest to " + str(average_values) + ', as requested in the argument -a')
...
def crop(video):
    ...
    if embryo_x and embryo_y and embryo_rad:
        ...
        LOGGER.warning("Couldn't detect circles in video. Cutting approximately around edges.")
        shape = video[0].shape
...
def run(video, args, video_metadata):
    ...
    if not args['slowmode']:
        ...
        ax, highest_freqs = PixelFourier(pixel_signals, times, empty_frames, frame2frame, args['threads'], pixel_num = 1000, plot = True)
    ...
    #must be removed from here and sent to the end, as if the script does not find any mask, it still runs the slow method
        ...
        highest_freqs2 = PixelFourier(all_pixel_sigs, times, empty_frames, frame2frame, args['threads'], plot = True)
...
# ## run the report and graphs functions
    ...
    if bpm_fourier is None:
        ...
        highest_freqs2 = PixelFourier(all_pixel_sigs, times, empty_frames, frame2frame, args['threads'], plot = False)   
        #Plot the density of fourier transform global maxima across pixels
        out_kde2 = os.path.join(out_dir, "pixel_rate_all.png")
        fig2, ax2 = plt.subplots(1,1,figsize=(10, 7))
        ax2, bpm_fourier = PixelFreqs(highest_freqs2, args['average'], peak_filter = False)
